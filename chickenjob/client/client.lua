...
---
		...
		if dist <= 2.5 then
		DrawText3D(startX, startY, startZ, "nhấn ~g~[E]~w~ để bắt gà")
		end
--
		if dist <= 0.5 then
...
if share == true then
	...
	if dist <= 1.0 then
	DrawText3D(chicken1Coords.x, chicken1Coords.y, chicken1Coords.z+0.5, "nhấn ~o~[E]~b~ để bắt gà")
		if IsControlJustPressed(0, Keys['E']) then 
		Caught1 = 1
		hewassplashed()
		end	
	elseif dist2 <= 1.0 then
		DrawText3D(chicken2Coords.x, chicken2Coords.y, chicken2Coords.z+0.5, "nhấn ~o~[E]~b~ để bắt gà")
		if IsControlJustPressed(0, Keys['E']) then 
		Caught2 = 1
		hewassplashed()
		end	
	elseif dist3 <= 1.0 then
		DrawText3D(chicken3Coords.x, chicken3Coords.y, chicken3Coords.z+0.5, "nhấn ~o~[E]~b~ để bắt gà")
		if IsControlJustPressed(0, Keys['E']) then 
		Caught3 = 1
		hewassplashed()
		end	
...
function packingg(stanowisko)
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			exports.pNotify:SendNotification(
				{
				text = "Tiếp tục đóng gói hoặc cất vào xe.",
				type = "success",
				timeout = 3000,
				layout = "bottomRight",
				queue = "chickenjob"
		...
		exports.pNotify:SendNotification(
			{
			text = "Không có thịt gà để đóng gói!",
			type = "success",
			timeout = 3000,
			layout = "bottomRight",
			queue = "chickenjob"
