...
function OpenBusinessMenu(business)
    ...
    ESX.TriggerServerCallback("esx_business:getStock", function(stock)
        local elements = {}
        table.insert(elements,{label = "Cổ phiếu: "..tostring(stock), value = ''})
        table.insert(elements,{label = 'Mua cổ phiếu - <span style="color:green;">$'..tostring(business["stock_price"])..'</span>', label_real = 'buystock', value = 1, type = 'slider', min = 1, max = 100})
        if business["owner"]==PlayerData.identifier then table.insert(elements,{label = 'Danh sách nhân viên', value = 'employeelist'}) end
        table.insert(elements,{label = "Thoát", value = 'exitmenu'})
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_boss',
        {
            ...
            if data.current.label_real == 'buystock' then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_confirm_stock', {
                    title    = ("Bạn có chắc chắn muốn mua %s cổ phiếu với giá %s?"):format(data.current.value,business["stock_price"]*data.current.value),
                    align    = 'bottom-right',
                    elements = {
                    ...
                    if data2.current.value == 'yes' then
                        ESX.TriggerServerCallback("esx_business:buyStock", function(success,time)
                            if success then
                                ESX.ShowNotification("Mua cổ phiếu thành công! Sẽ bán được sau "..disp_time(time))
                            else ESX.ShowNotification("~r~Bạn không có đủ tiền") end
            ...
            elseif data.current.value == 'sellbusiness' then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_confirm_sellbusiness', {
                    title    = ("Bạn có chắc chắn muốn bán %s với %s?"):format(business["name"],math.floor(business["price"]*Config.sell_percentage)),
                    align    = 'bottom-right',
                    elements = {
                        {label = 'No',  value = 'no'},
                        {label = 'Yes', value = 'yes'},
                    ...
                    if data2.current.value == 'yes' then
                        ESX.TriggerServerCallback("esx_business:sellBusiness", function(success)
                            if success then
                                ESX.ShowNotification("Bạn đã bán bất động sản ~g~$"..math.floor(business["price"]*Config.sell_percentage))
            ...
            elseif data.current.value=="employeelist" then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_choice', {
                    ...
                    align    = 'bottom-right',
                    elements = {
                        {label = 'Tuyển nhân viên',  value = 'hire'},
                        {label = 'Sa thải nhân viên', value = 'fire'},
                    }
                }, function(data2, menu2)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_buy',
    {
        ...
        if data.current.value == 'yes' then
            ESX.TriggerServerCallback("esx_business:buyBusiness",function(status)
                if status==0 then
                    ESX.ShowNotification("Bạn đã mua thành công ~b~"..business["name"].."~s~!")
                elseif status==1 then
                    ESX.ShowNotification("~r~"..business["name"].."~s~ đã có người sở hữu rồi!")
                else
                    ESX.ShowNotification("Bạn không có đủ tiền để mua ~b~"..business["name"].."~s~!")
                end
            end,business["id"])
        end
            ...
            if GetDistanceBetweenCoords(ppos,x,y,z,false)<Config.draw_distance then
                if business.owner==nil then
                    if GetDistanceBetweenCoords(ppos,x,y,z,false)<2.0 and not buy then
                        ESX.ShowHelpNotification('Nhấn E để mua bất động sản')
                ...
                elseif business.owner==PlayerData.identifier or business.isemployee then
                    ...
                    if GetDistanceBetweenCoords(vector3(xx,yy,zz),GetEntityCoords(ped),false)<1.0 and not boss then
                        ESX.ShowHelpNotification('Nhấn E để mở menu bất động sản')
