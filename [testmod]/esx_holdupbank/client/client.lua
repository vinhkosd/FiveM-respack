...
AddEventHandler('esx_holdupbank:robberycomplete', function(robb)
	...
	TriggerEvent("XNL_NET:AddPlayerXP", math.floor(Banks[bank].reward/20))
	TriggerServerEvent("AddXp", math.floor(Banks[bank].reward/20))
	bank = ""
	secondsRemaining = 0
	incircle = false
end)
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						...
						if IsControlJustReleased(1, 51) then
							if exports.XNLRankBar:Exp_XNL_GetCurrentPlayerLevel() >= 5 then
								TriggerServerEvent('esx_holdupbank:rob', k)
							else
								ESX.ShowNotification('Bạn phải đạt cấp 5 để thực hiện')
							end
						end
