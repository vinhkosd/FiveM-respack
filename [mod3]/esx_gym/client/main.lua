...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để thuê xe đạp')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Bạn đã thuê xe đạp")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để mở menu Gym')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để tập tay của bạn')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Chuẩn bị tập...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Bạn cần nghỉ ngơi vài phút trước khi tập bài khác")
							TriggerEvent("XNL_NET:AddPlayerXP",40)
							TriggerServerEvent("AddXp", 40)
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Bạn cần có thẻ thành viên Gym")
						end
					elseif training == true then
						ESX.ShowNotification("Đến lúc nghỉ ngơi rồi...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để đu xà nhé')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để làm vài cái chống đẩy')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để tạp Yoga')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Nhấn E để làm vài cái gập bụng')
...
function CheckTraining()
	if resting == true then
		ESX.ShowNotification("Bạn đang nghỉ ngơi")
	...
	if resting == false then
		ESX.ShowNotification("Bạn khỏe như trâu rồi, tập tiếp nào!")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Giờ làm việc', value = 'hours'},
				{label = 'Thành Viên', value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Chúng tôi mở cửa cả ngày lẫn đêm.")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Protein Shake ($50)', value = 'protein_shake'},
				{label = 'Water ($15)', value = 'water'},
				{label = 'Sportlunch ($50)', value = 'sportlunch'},
				{label = 'Powerade ($40)', value = 'powerade'},
				{label = 'Bandage ($1000)', value = 'bandage'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Membership ($5000)', value = 'membership'},
            }
        },
        function(data, menu)
