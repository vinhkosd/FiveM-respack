...
function OpenCloakroomMenu()
	...
	if Config.EnableNonFreemodePeds then
		table.insert(elements, {label = 'Police 1', value = 'freemode_ped', maleModel = 's_m_y_sheriff_01', femaleModel = 's_m_y_sheriff_01'})
		table.insert(elements, {label = 'Police 2', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_m_y_cop_01'})
		table.insert(elements, {label = 'Police 3', value = 'freemode_ped', maleModel = 'u_m_y_juggernaut_01', femaleModel = 'u_m_y_juggernaut_01'})
	end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		align    = 'top-left',
			...
			{label = 'Tù ngục',               value = 'jail_menu'},
			--{label = 'Truy Nã',               value = 'wanted'},
			--{label = 'Cứu người',               value = 'revive'},
  ...
  if data.current.value == 'jail_menu' then
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = 'Cải tạo', value = 'communityservice'},
				{label = _U('license_check'), value = 'license'}
				--{label = _U('weapon'),			value = 'weapon'},		                                               
			}

				if PlayerData.job.grade_name == 'boss' then
				table.insert(elements, { label = 'Cấp Giấy Sử Dụng Súng', value = 'weapon' })					
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
	                ...
	                elseif action == 'weapon' then
						TriggerServerEvent('esx_policejob:message', GetPlayerServerId(closestPlayer), _U('being_weapon'))
		...
		elseif data.current.value == 'wanted' then
			menu.close()
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wanted', {title = "ID người cần truy nã"}, function(data2, menu2)
				local targetId = data2.value
				if data2.value == nil then
					ESX.ShowNotification('Bạn chưa nhập id người chơi bị truy nã')
				else
					menu2.close()
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wanted2', {title = "Thời gian truy nã"}, function(data3, menu3)
						local timeWanted = data3.value
						if timeWanted == nil then
							ESX.ShowNotification("Bạn chưa nhập thời gian truy nã")
						else
							menu3.close()
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wanted3', {title = "Lý do truy nã"}, function(data4, menu4)
								local reason = data4.value
								if reason == nil then
									ESX.ShowNotification("Bạn chưa nhập lý do truy nã")
								else
									TriggerServerEvent("esx-wanted:aaaaaaaa", targetId, timeWanted, reason)
									menu4.close()
								end
		...
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
							...
							ESX.TriggerServerCallback('2785e659-a064-451c-b65d-84d47bdc9086', function(quantity)
							if quantity > 0 then
								...
								if IsPedDeadOrDying(closestPlayerPed, 1) then
									...
									exports['mythic_notify']:DoLongHudText('inform', 'Dang duoc cuu')
									...
									TriggerServerEvent('28f6ef64-ba47-47e5-9903-286df481fdb1', 'medikit')
									TriggerServerEvent('fd305b6b-28cb-4541-8830-cb0174031c50', GetPlayerServerId(closestPlayer))

								else
									exports['mythic_notify']:DoLongHudText('inform', _U('player_not_unconscious'))
								end
							else
								exports['mythic_notify']:DoLongHudText('inform', _U('not_enough_medikit'))
							end
...
RegisterNetEvent('esx_policejob:doesHandCuff')
AddEventHandler('esx_policejob:doesHandCuff', function(by)
	TriggerServerEvent('esx_policejob:returnHandcuff', IsHandcuffed, by)
end)
