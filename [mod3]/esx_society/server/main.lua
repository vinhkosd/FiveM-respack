...
ESX.RegisterServerCallback('esx_society:getEmployees2', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT firstname, lastname, identifier, job2, job2_grade FROM users WHERE job2 = @job ORDER BY job2_grade DESC', {
			['@job'] = society
		}, function (results)
		...
		MySQL.Async.fetchAll('SELECT name, identifier, job2, job2_grade FROM users WHERE job2 = @job ORDER BY job2_grade DESC', {
			['@job'] = society
		}, function (result)
			local dumped = ESX.DumpTable(results)
			print(dumped)
			local employees = {}
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'chief'
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and (xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'chief') then
		return true
	else
