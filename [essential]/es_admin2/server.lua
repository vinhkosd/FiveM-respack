...
function isBanned(id)
	if bannedTable[id] ~= nil then
		if bannedTable[id].expire < os.time() and bannedTable[id].expire ~= 0 then
			removeBan(id)
			return false
		else
			...
			elseif t == "money" then
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						...
						TriggerClientEvent('chat:addMessage', Source, {
							args = {"^1Bìu", "Set Cái Máu Lồn Con Chồn"}
						})							
						end)
					else
						TriggerClientEvent('chat:addMessage', Source, {
...
RegisterCommand('setblackmoney', function(source, args, raw)
	local player = tonumber(args[1])
	local money = tonumber(args[2])
	if args[1] then
		if (player and GetPlayerName(player)) then
			if money then
				TriggerEvent("es:getPlayerFromId", player, function(user)
					if(user)then
						...
						TriggerClientEvent('chat:addMessage', player, {
							args = {"^1SYSTEM", "Your black money has been set to: ^2^*$" .. money}
						})
					end
				end)
			else
