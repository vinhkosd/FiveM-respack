...
Citizen.CreateThread(function()
		...
		local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
					...
					AddTextComponentString("Nơi bán cá")
		...
		local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
					...
					BeginTextCommandSetBlipName("STRING")
					AddTextComponentString("Nơi bán thịt rùa biển")
					EndTextCommandSetBlipName(blip2)
					
				blip2 = AddBlipForRadius(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z , 100.0) -- you can use a higher number for a bigger zone
		...
		local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
					...
					BeginTextCommandSetBlipName("STRING")
					AddTextComponentString("Nơi bán thịt cá mập")
					EndTextCommandSetBlipName(blip3)
					
				blip3 = AddBlipForRadius(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z , 100.0) -- you can use a higher number for a bigger zone
		...
		for _, info in pairs(Config.MarkerZones) do
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString("Nơi cho thuê thuyền")
	...
	while true do
		...
		if fishing then
			...
			if fishing then
				...
				if pos.y >= 1000 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 or IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.SellFish.x, Config.SellFish.y, Config.SellFish.z, true) <= 3 then
			...
			ESX.ShowNotification("~r~Dang ban")
			Citizen.Wait(4000)
		end
