    ...
    while true do
        ...
        for k,v in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance and k == 'Pimp' then
                letSleep = false
                DrawText3Ds(v.Pos.x, v.Pos.y, v.Pos.z+1.0, "Nhan ~b~[E]~w~ de dat hang")
...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    ...
    exports['mythic_notify']:DoLongHudText('inform', "Thanh Xuân đã có vị trí trên GPS, Đến *** cô ấy đi.")
...
RegisterNUICallback("ChooseTatiana", function (data, callback)
    ...
    exports['mythic_notify']:DoLongHudText('inform', "Phương Ly đã có vị trí trên GPS, Đến *** cô ấy đi.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseBootylicious", function (data, callback)
    ...
    exports['mythic_notify']:DoLongHudText('inform', "Trân Trân đã có vị trí trên GPS, Đến *** cô ấy đi.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseVennesa", function (data, callback)
    ...
    exports['mythic_notify']:DoLongHudText('inform', "Ngọc Trinh đã có vị trí trên GPS, Đến *** cô ấy đi.")
    OnRouteToHooker = true
end)
...
AddEventHandler("sawu_hookers:ChosenHooker", function(model)
    if HookerSpawned then
        exports['mythic_notify']:DoLongHudText('error', "Bạn đã chọn cô em khác rồi mà!")
    else
        HookerSpawned = true
        CreateHooker(model)
        Citizen.CreateThread(function()
                ...
                if OnRouteToHooker then 
                    if GetDistanceBetweenCoords(Coords, Config.Hookerspawns[spawn].x,Config.Hookerspawns[spawn].y,Config.Hookerspawns[spawn].z, true) < Config.DrawMarker then
                        ...
                        if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                            DrawText3Ds(Config.Hookerspawns[spawn].x,Config.Hookerspawns[spawn].y,Config.Hookerspawns[spawn].z+1.0, 'Nhan [~b~E~w~] de goi gai ban dam')
                ...
                if HookerInCar then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and not IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                        ...
                        if IsVehicleStopped(vehicle) then
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, 'Nhan [~b~E~w~] de xem cac dich vu, nhan [~r~H~w~] de duoi gai ban dam di')
                            if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, 'Hay lai xe den mot noi vang ve. Noi ma chi co doi ta :D ')
                        end
                    end
                end
                if letSleep then
