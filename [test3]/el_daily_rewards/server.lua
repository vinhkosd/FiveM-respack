...
AddEventHandler("free:log", function(it, xPlayer)
	...
	local text
	if it.type==1 and it.value then
		text = "người chơi "..xPlayer.name.." đã nhận được "..it.value.."$ từ quà tặng hàng ngày"
	elseif it.type==2 and it.item and it.count then
		text = "người chơi "..xPlayer.name.." đã nhận được "..it.count.." x "..it.item.." từ quà tặng hàng ngày"
	elseif it.type==3 and it.weapon and it.ammo then
		text = "người chơi "..xPlayer.name.." đã nhận được "..it.weapon.." từ quà tặng hàng ngày"
	end
