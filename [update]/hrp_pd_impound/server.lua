	...
	MySQL.Async.execute('INSERT INTO `h_impounded_vehicles` VALUES (@plate, @officer, @mechanic, @releasedate, @fee, @reason, @notes, CONCAT(@vehicle), @identifier, @hold_o, @hold_m)',
		{
			...
			if (rowsChanged == 0) then
				TriggerClientEvent('esx:showNotification', _source, 'Không thể tạm giữ')
			else
				TriggerClientEvent('esx:showNotification', _source, 'Đã tạm giữ phương tiện')
				MySQL.Sync.execute("UPDATE owned_vehicles SET impound = 1 WHERE plate=@plate", {['@plate'] = _form.plate})
			end
	end)
end)
...
AddEventHandler('HRP:Impound:UnimpoundVehicle', function (plate)
	...
	if (xPlayer.getMoney() < veh[1].fee) then
		...
		MySQL.Sync.execute("UPDATE owned_vehicles SET impound = 0 WHERE plate=@plate", {['@plate'] = plate})
