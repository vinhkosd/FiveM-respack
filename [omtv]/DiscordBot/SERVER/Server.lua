...
-- System Infos
PerformHttpRequest(DiscordKhoiDongServer, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**Server Đã Khởi Chạy ! , Mời Anh Em Vào Chơi Ngay Nàoo ! Chúc Anh Em Chơi Game Vui Vẻ**'}), { ['Content-Type'] = 'application/json' })

AddEventHandler('playerConnecting', function()
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```css\n' .. GetPlayerName(source) .. ' đăng nhập vào server\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```fix\n' .. GetPlayerName(source) .. ' thoát server (' .. Reason .. ')\n```', SystemAvatar, false)
end)
...
AddEventHandler('DiscordBot:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
	...
	if External then
		if WebHook:lower() == 'trò chuyện' then
			WebHook = DiscordWebhookChat
		elseif WebHook:lower() == 'máy chủ' then
			WebHook = DiscordWebhookSystemInfos
		elseif WebHook:lower() == 'giết người' then
...
PerformHttpRequest('https://raw.githubusercontent.com/Flatracer/FiveM_Resources/master/' .. GithubResourceName .. '/VERSION', function(Error, NewestVersion, Header)
	PerformHttpRequest('https://raw.githubusercontent.com/Flatracer/FiveM_Resources/master/' .. GithubResourceName .. '/CHANGES', function(Error, Changes, Header)
		if(NewestVersion == nil) then
			print('Cant get version info ' .. GithubResourceName)
			return nil
		end
