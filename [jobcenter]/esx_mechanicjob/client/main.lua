...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TrigerServerEvent('esx_mechanicjob:onNPCJobCompleted')
	end
end
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						TrigerServerEvent('esx_society:removeVehicleFromGarage', 'mechanic', vehicleProps)
				...
				if Config.EnablePlayerManagement and ESX.PlayerData.job and (ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'chief' or ESX.PlayerData.job.grade_name == 'experimente') then
					table.insert(elements, {label = 'Xe Mod', value = '610DTM'})
				end
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_harvest', {
			...
			if data.current.value == 'gaz_bottle' then
				TrigerServerEvent('esx_mechanicjob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				TrigerServerEvent('esx_mechanicjob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				TrigerServerEvent('esx_mechanicjob:startHarvest3')
			end
		end, function(data, menu)
			menu.close()
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'fix_kit' then
				TrigerServerEvent('esx_mechanicjob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TrigerServerEvent('esx_mechanicjob:startCraft3')
			end
		end, function(data, menu)
			menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TrigerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			isBusy = true
				...
				Citizen.CreateThread(function()
				exports['progressBars']:startUI(20000, "Đang sửa")	
					Citizen.Wait(20000)
					...
					TriggerEvent("XNL_NET:AddPlayerXP", math.random(10, 20))
					TriggerServerEvent("AddXp", math.random(10, 20))
				...
				if NPCOnJob then
					if NPCTargetDeleterZone then
						...
						if CurrentlyTowedVehicle == NPCTargetTowable then
							...
							TrigerServerEvent('esx_mechanicjob:onNPCJobMissionCompleted')
							StopNPCJob()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TrigerServerEvent('esx_mechanicjob:getStockItem', itemName, count)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TrigerServerEvent('esx_mechanicjob:putStockItems', itemName, count)
...
AddEventHandler('esx_mechanicjob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TrigerServerEvent('esx_mechanicjob:stopCraft')
		TrigerServerEvent('esx_mechanicjob:stopCraft2')
		TrigerServerEvent('esx_mechanicjob:stopCraft3')
	elseif zone == 'Garage' then
		TrigerServerEvent('esx_mechanicjob:stopHarvest')
		TrigerServerEvent('esx_mechanicjob:stopHarvest2')
		TrigerServerEvent('esx_mechanicjob:stopHarvest3')
	end
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TrigerServerEvent('esx_society:putVehicleInGarage', 'mechanic', vehicleProps)
						...
						then
							TrigerServerEvent('esx_service:disableService', 'mechanic')
						end
