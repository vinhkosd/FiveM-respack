...
function ActivateBlips()
    ...
    AddTextComponentString("Khu vực lấy xe")
    EndTextCommandSetBlipName(blip)
end
function ActivateBlips2()
	local blip = AddBlipForCoord(_Impound.StoreLocation.X, _Impound.StoreLocation.Y, _Impound.StoreLocation.Z)
	SetBlipScale(blip, 0.9)
	SetBlipDisplay(blip, 4)
	SetBlipSprite(blip, 430)
	SetBlipColour(blip, 3)
	SetBlipAsShortRange(blip, true)
	BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Khu vực tạm giữ xe")
    EndTextCommandSetBlipName(blip)
end
...
AddEventHandler('HRP:Impound:VehicleUnimpounded', function (data, index)
	...
	_ESX.ShowNotification("Phương tiện của bạn với biển số: " .. data.plate .. " đã được thả!")
...
AddEventHandler('HRP:Impound:CannotUnimpound', function ()
	_ESX.ShowNotification("Phương tiện của bạn không thể được thả, liệu bạn có đủ tiền ?");
end)
...
function ShowImpoundMenu (action)
	...
	if (IsPedInAnyVehicle(GetPlayerPed(PlayerId()))) then
		_ESX.ShowNotification("Hãy xuống xe")
		return
	end
	...
	if (vehicle ~= nil) then
		...
		if(_VehicleAndOwner == nil) then
			_ESX.ShowNotification('Không tìm thấy chủ phương tiện, không thể tạm giữ');
			return
		end
		...
		_ESX.ShowNotification('Không có phương tiện bên cạnh');
	end
		...
		if(_DependenciesLoaded) then
			...
			if (GetDistanceBetweenCoords(_Impound.RetrieveLocation.X, _Impound.RetrieveLocation.Y, _Impound.RetrieveLocation.Z,
				...
				if (_CurrentAction ~= "retrieve") then
					...
					_ESX.ShowHelpNotification("Nhấn [E] để xử lý biên bản");
			...
			elseif (GetDistanceBetweenCoords(_Impound.StoreLocation.X, _Impound.StoreLocation.Y, _Impound.StoreLocation.Z,
				...
				if (_CurrentAction ~= "store" and (_XPlayer.job.name == "police" or _XPlayer.job.name == "mecano")) then
					...
					_ESX.ShowHelpNotification("Nhấn [E] để tạm giữ phương tiện");
