...
Citizen.CreateThread(function()
	while true do
		...
		--[[
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 30.0 then
...
Citizen.CreateThread(function()
	...
	for i=1, #Config.CircleZones, 1 do
		...
		local blip = AddBlipForCoord(Config.CircleZones[i].x, Config.CircleZones[i].y, Config.CircleZones[i].z)
		SetBlipSprite (blip, Config.CircleZones[i].id)
		SetBlipDisplay(blip, 4)
		SetBlipColour (blip, Config.CircleZones[i].color)
		SetBlipScale  (blip, Config.CircleZones[i].scale)
		SetBlipAsShortRange(blip, true)

		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString(Config.CircleZones[i].name)
		EndTextCommandSetBlipName(blip)
	end
