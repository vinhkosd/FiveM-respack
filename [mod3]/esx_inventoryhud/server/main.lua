		...
		local targetXPlayer = ESX.GetPlayerFromId(target)
		 ...
		 if type == "item_standard" then
            ...
            if itemCount > 0 and sourceItem.count >= itemCount then
                if targetItem.limit == -1 or targetXPlayer.canCarryItem(item, count) then
...
AddEventHandler("suku:SellItemToPlayer",function(source, type, item, count)
    ...
    if type == "item_standard" then
        ...
        if targetItem.limit == -1 or xPlayer.canCarryItem(item, count) then
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Bạn đã mua '..count.." "..list[i].label..' với số tiền: '..totalPrice..' $' })
					else
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Túi đầy!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Bạn đã mua '..list[i].label..' với số tiền: '..totalPrice..' $' })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Bạn không đủ tiền!' })
						end
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Bạn đã sở hữu vũ khí này!' })
					end
				end
            end
        else
			...
			if list[i].name == item then
				...
				if targetWeapon then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Bạn đã mua '..count.." "..list[i].label })
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Bạn không đủ tiền!' })
					end
				else
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Bạn không sở hữu loại súng sử dụng loại đạn này!' })
				end
            end
        end
    end
...
AddEventHandler('suku:buyLicense', function ()
	...
	if xPlayer.get('money') >= Config.LicensePrice then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Bạn đã mua giấy phép sử dụng súng.' })
