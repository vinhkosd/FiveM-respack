...
AddEventHandler('esx_communityservice:finishCommunityService', function(source)
	...
	isSentenced = false
	actionsRemaining = 0
	for i=1, #binBagProps, 1 do --remove all binbag khi xong nhiem vu
		if GetDistanceBetweenCoords(coords, GetEntityCoords(binBagProps[i]), false) < 3 then-- tìm vật thể gần người chơi nhất
			local playerPed = PlayerPedId()
			if binBagProps[i] and IsPedOnFoot(playerPed) then
				print('found binbag nearById'..i)
				ESX.Game.DeleteObject(binBagProps[i])
				table.remove(binBagProps, i)
			else
				print('cant find binbag')
			end
		end
	end
end)
...
function RemoveAction(action)
	...
	if action_pos ~= -1 then
		...
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)--get player coords
		for i=1, #binBagProps, 1 do
			if GetDistanceBetweenCoords(coords, GetEntityCoords(binBagProps[i]), false) < 3 then-- tìm vật thể gần người chơi nhất
				nearbyObject, nearbyID = binBagProps[i], i
			end
		end
		if nearbyObject and IsPedOnFoot(playerPed) then
			print('found binbag nearById'..nearbyID)
			ESX.Game.DeleteObject(nearbyObject)
			table.remove(binBagProps, nearbyID)
		else
			print('cant find binbag')
		end
	else
...
--{ r = 50, g = 50, b = 204 }
		...
		local coordBinBag = vector3(availableActions[i].coords.x, availableActions[i].coords.y, availableActions[i].coords.z)
		if ValidateBinBagCoord(coordBinBag) then
			ESX.Game.SpawnLocalObject('prop_rub_binbag_03b',coordBinBag , function(obj)
				PlaceObjectOnGroundProperly(obj)
				FreezeEntityPosition(obj, true)
	
				table.insert(binBagProps, obj)
			end)
		end
...
function ValidateBinBagCoord(binBagCoord)
	local validate = true

	for k, v in pairs(binBagProps) do
		if GetDistanceBetweenCoords(binBagCoord, GetEntityCoords(v), true) < 5 then
			validate = false
		end
	end
...
function DisableViolentActions()
	...
	if IsDisabledControlJustPressed(2, 37) then --if Tab is pressed, send error message
		SetCurrentPedWeapon(playerPed,GetHashKey("WEAPON_UNARMED"),false) -- if tab is pressed it will set them to unarmed (this is to cover the vehicle glitch until I sort that all out)
	end

	if IsDisabledControlJustPressed(0, 106) then --if LeftClick is pressed, send error message
		SetCurrentPedWeapon(playerPed,GetHashKey("WEAPON_UNARMED"),false) -- If they click it will set them to unarmed
	end
