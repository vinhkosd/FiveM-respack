	...
	if Stores[robb] then
		...
		if rob == false then
			...
			if(cops >= Config.RequiredCopsRob)then
				...
				TriggerEvent('trew_hud_ui:adminNotifyAllPlayers', {'Thông báo có vụ cướp tiệm trang sức', 'Cửa hàng '..store.nameofstore..' hiện đang bị cướp bởi '..xPlayer.name..' đề nghị mọi người tránh xa', 'red'})
