		...
		if rob == false then
		  ...
		  if xPlayer.getInventoryItem('drill').count >= 1 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				TriggerEvent('trew_hud_ui:adminNotifyAllPlayers', {'Thông báo có vụ cướp ngân hàng', 'Ngân hàng '..bank.nameofbank..' hiện đang bị cướp bởi '..xPlayer.name..' đề nghị mọi người tránh xa', 'red'})
				...
				TriggerClientEvent('esx:showNotification', source, _U('min_six_police') .. Config.NumberOfCopsRequired)
			end
		end
		else
