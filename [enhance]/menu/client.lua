...
RegisterNUICallback('smoke', function(data)
  playerPed = GetPlayerPed(-1);
		if(not IsPedInAnyVehicle(playerPed)) then
			if playerPed then
				if playing_emote == false then
...
RegisterNUICallback('kiss', function(data)
  playerPed = GetPlayerPed(-1);
		if(not IsPedInAnyVehicle(playerPed)) then
			if playerPed then
				if playing_emote == false then
          LoadAnim("anim@mp_player_intcelebrationfemale@blow_kiss")
					TaskPlayAnim(GetPlayerPed(-1), "anim@mp_player_intcelebrationfemale@blow_kiss", "blow_kiss", 2.0, 2.0, -1, 0, 0, false, false, false)
          RemoveAnimDict("anim@mp_player_intcelebrationfemale@blow_kiss")
					playing_emote = true
				end
			end
		end
