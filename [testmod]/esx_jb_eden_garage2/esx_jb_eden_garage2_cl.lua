...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Lấy xe ra" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Đổi tên xe" , value = 'rename_vehicle'})
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Xe của bạn đang ở trong nhà để xe')
					else
						TriggerEvent('esx:showNotification', 'Vous ne pouvez pas stocker ce véhicule')
					end
...
function ReturnVehicleMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			...
			table.insert(elements, {label ="Không có xe để tẩu thoát" , value = nil})
		end
