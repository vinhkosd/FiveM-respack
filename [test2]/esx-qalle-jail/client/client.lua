...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), Config.Teleports["Sảnh trước"])
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			local distancePrisonWork = GetDistanceBetweenCoords(pedLocation, Config.Teleports["Khu vực lao động"]["x"], Config.Teleports["Khu vực lao động"]["y"], Config.Teleports["Khu vực lao động"]["z"], true)
			local distanceBoilingBroke = GetDistanceBetweenCoords(pedLocation, Config.Teleports["Sảnh trước"]["x"], Config.Teleports["Sảnh trước"]["y"], Config.Teleports["Sảnh trước"]["z"], true)
			local distanceJail = GetDistanceBetweenCoords(pedLocation, Config.Teleports["Phòng giam"]["x"], Config.Teleports["Phòng giam"]["y"], Config.Teleports["Phòng giam"]["z"], true)
			local distaceThamnuoi = GetDistanceBetweenCoords(pedLocation, Config.Teleports["Phòng thăm nuôi"]["x"], Config.Teleports["Phòng thăm nuôi"]["y"], Config.Teleports["Phòng thăm nuôi"]["z"], true)
			if (distancePrisonWork > 40) and (distanceBoilingBroke > 20) and (distanceJail > 20) and (distaceThamnuoi > 20) then
				TriggerServerEvent("esx-qalle-jail:vuotNguc")
				ESX.Game.Teleport(PlayerPedId(), Config.Teleports["Phòng giam"])
				ESX.ShowNotification('Bạn bị cải tạo bổ sung thêm 10 phút, hãy sống và làm người')
				jailTime = jailTime + 11
			end
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									TriggerEvent("XNL_NET:AddPlayerXP", jailTime*100)
									TriggerServerEvent("AddXp", jailTime*100)
								end
