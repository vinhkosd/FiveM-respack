...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			if exports.XNLRankBar:Exp_XNL_GetCurrentPlayerLevel() >=3 then
				HasAlreadyEnteredMarker = true
				TriggerEvent('esx_weaponshop:hasEnteredMarker', currentZone)
			else
				ESX.ShowNotification('Bạn phải đạt cấp độ 3 để mua vũ khí')
			end
		end
					...
					if Config.LicenseEnable and Config.Zones[CurrentActionData.zone].Legal then
						ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
							if hasWeaponLicense then
								...
								ESX.ShowNotification("~r~Bạn không có giấy phép sử dụng vũ khí , hãy gặp CST để mua")
							end
...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(10000)
		if IsPedArmed(PlayerPedId(), 7) and exports.XNLRankBar:Exp_XNL_GetCurrentPlayerLevel() <2 then
