	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " bị giam giữ trong " .. jailTime .. " phút!")
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "Nhà tù",  Firstname .. " " .. Lastname .. " bị giam vì lý do: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
		...
		TriggerClientEvent("esx:showNotification", src, "Bạn không có quyền làm điều này!")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "Nhà tù",  Firstname .. " " .. Lastname .. " bị giam vì lý do: " .. jailReason .. " trong " .. jailTime .. "phút"}, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " bị giam giữ trong " .. jailTime .. " phút!")
end)
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
	...
	TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " được thả ra tù!")
end)

RegisterServerEvent("esx-qalle-jail:vuotNguc")
AddEventHandler("esx-qalle-jail:vuotNguc", function()
	local name = GetPlayerName(source)
	TriggerClientEvent("esx:showNotification", -1, "~r~"..name.." đã tổ chức vượt ngục và bị bắt")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Cảm ơn, đây là chút tiền cho bạn mua đồ ăn!")
end)
